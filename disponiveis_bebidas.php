...
$results = $mysqli->query("select * from bebida")or die($mysqli->error);
?>
  <head>
  ...
  <body>
    <div id="container">
      <table>
        ...
        <tr>
          <form id="add_bebida" enctype="multipart/form-data">
            <input type="hidden" name="op" id="op" value="add_bebida">
            ...
            <td><input id="preco" type="number" name="preco" placeholder="Preço"></td>
            <td>
              <button onclick='var form =$("#add_bebida").serialize();
                              var settings = {
                                "async": true,
                                "crossDomain": true,
      ...
      while($row = $results->fetch_assoc()) {
            ...
            print '<td><img src="https://www.solodev.com/assets/lazy-load-with-echo/image-loader.gif"  data-echo="img/bebidas/'.strtolower($row["nome"]).'.jpg"></td>';
            print '<td>'.$row["idbebida"]. " - ".$row["nome"].'</td>';
            print '<td>'.$row["descricao"].'</td>';
            print '<td>R$'.$row["preco"].'</td>';
            print '<td><button onclick="$.ajax({url: \'https://domluiggi.ahto.digital/apir.php?op=deleta_bebida&id=' . $row['idbebida'] .'\'});window.location.reload(true);">Apagar</button></td>';
            print '</tr>';
      } ?>
